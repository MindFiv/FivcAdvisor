...
class ToolsBundle:
    ...
    Attributes:
        ...
        tools: Mapping of tool_name -> AgentTool
    ...
    """Mapping of tool_name -> AgentTool"""
    ...
    def add_tool(self, tool: AgentTool) -> None:
        ...
        if tool.tool_name in self.tools:
            raise ValueError(
                f"Tool {tool.tool_name} already exists in bundle {self.bundle_name}"
            )
    ...
    def add_tools(self, tools: List[AgentTool]) -> None:
        ...
        Args:
            tools: List of AgentTools to add
        """
        for tool in tools:
            self.add_tool(tool)
    ...
    def get_tool(self, tool_name: str) -> Optional[AgentTool]:
        ...
        Returns:
            The AgentTool or None if not found
        """
    ...
    def get_all_tools(self) -> List[AgentTool]:
        ...
        Returns:
            List of all AgentTools in this bundle
        """
...
class ToolsBundleManager:
    ...
    def add_tool_to_bundle(self, bundle_name: str, tool: AgentTool) -> None:
        """
    ...
    def get_bundle_tools(self, bundle_name: str) -> List[AgentTool]:
        """
        ...
        Returns:
            List of AgentTools in the bundle
        """
        bundle = self.get_bundle(bundle_name)
        if bundle:
    ...
    def expand_tools(
        ...
        expanded_tool_names = {t.tool_name for t in tools}

        for tool in tools:
            bundle = self.get_bundle_by_tool(tool.tool_name)
            if bundle is None:
                continue
            ...
            for bundle_tool in bundle.get_all_tools():
                if bundle_tool.tool_name not in expanded_tool_names:
                    expanded_tools.append(bundle_tool)
                    expanded_tool_names.add(bundle_tool.tool_name)
